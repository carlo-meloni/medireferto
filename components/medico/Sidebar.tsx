  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 py-4 px-2 flex flex-col gap-1">
        {navItems.map((item) => {
          const active = item.href === '/medico' ? pathname === '/medico' : pathname.startsWith(item.href);
          return (
            <Link
              key={item.href}
              href={item.href}
      ...
      <div className={`px-2 py-4 border-t border-zinc-200 flex flex-col gap-1 shrink-0`}>
        <Link
          ...
          className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900 transition ${
            collapsed ? 'justify-center' : ''
          }`}
